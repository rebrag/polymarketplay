...
POLY_URL = "https://polymarket.com/event/cbb-uconn-fl-2025-12-09"
...
def process_single_event(event: Any):
    ...
    print(f"\n[PRICE_CHANGE #{price_change_count}] ts={ts}")
    for pc in pcs:
        aid = pc.get("asset_id")
        label = ASSETS.get(aid, aid)
        bb = r3(pc.get("best_bid"))
        ba = r3(pc.get("best_ask"))
        mid = r3((bb + ba) / 2) if bb is not None and ba is not None else None
        print(
            f"  asset={aid}\n"
            f"    label={label}\n"
            f"    best_bid={bb} best_ask={ba} mid={mid}"
        )
